...
type OutfitDetailProps = {
  outfit: Outfit;
  onBack: () => void;
  onAddToCart: (item: Outfit, size: string, color: string) => void;
};

const sizes = ['S', 'M', 'L', 'XL'];

export const OutfitDetailPage: React.FC<OutfitDetailProps> = ({ outfit, onBack, onAddToCart }) => {
  const [selectedSize, setSelectedSize] = React.useState('M');
  const [selectedColor, setSelectedColor] = React.useState(outfit.variants[0].color);
  const [currentImage, setCurrentImage] = React.useState(outfit.variants[0].image);
  const [isAdded, setIsAdded] = React.useState(false);

  const handleColorSelect = (color: string) => {
    setSelectedColor(color);
    const newVariant = outfit.variants.find(v => v.color === color);
    if (newVariant) {
  ...
  const handleAddToCartClick = () => {
    onAddToCart(outfit, selectedSize, selectedColor);
    setIsAdded(true);
    setTimeout(() => {
        setIsAdded(false);
    }, 2000);
  ...
  return (
    <div className="w-full min-h-screen grid grid-cols-1 lg:grid-cols-2 animate-fade-in">
      <div className="relative h-[50vh] lg:h-screen bg-[#E5DCE9]">
        ...
        >
          &larr; Back to Latest Drops
        </button>
      </div>

      <div className="flex flex-col justify-center p-8 lg:p-16 overflow-hidden">
        <div className="animate-slide-in-up" style={{ animationDelay: '200ms' }}>
            <h1 className="text-4xl font-bold tracking-tight">{outfit.name}</h1>
        </div>
        ...
        <div className="mt-12 flex items-center justify-between animate-slide-in-up" style={{ animationDelay: '600ms' }}>
            <span className="text-4xl font-['Roboto_Mono',_monospace] font-bold">${outfit.price.toFixed(2)}</span>
            <button 
